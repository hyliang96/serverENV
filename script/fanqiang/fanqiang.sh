...
tfq_start_() {
    ...
    export http_proxy="http://127.0.0.1:$fq_port"
    export https_proxy="http://127.0.0.1:$fq_port"
    export ftp_proxy="http://127.0.0.1:$fq_port"
...
if [ "$fq_tool" = ss ]; then
    ...
    alias tfq_start="tfq_start_ $polipo_port"
elif [ "$fq_tool" = v2 ]; then
    alias fq='v2'     # 用v2ray翻墙
    alias tfq_start="tfq_start_ $v2_port"
else
